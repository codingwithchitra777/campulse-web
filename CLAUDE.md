...
`pages/alerts` (`GET/POST/DELETE /api/alerts`) sets one-shot **price alerts** delivered via the user's **linked Telegram** — a backend daemon polls quotes and messages the chat when a symbol crosses the target. The page warns (`deliverable` flag) when no Telegram is linked, linking to `/settings`. Direction (above/below) is inferred from the current price server-side. `ApiService`: `getAlerts`/`createAlert`/`removeAlert`; model `PriceAlert`. Analytics/Watchlist/Alerts sit under a **Tools** sidebar group (real, free); `AI Coach` + `Journal` remain the only `Premium 👑` teasers.
